...
Begin VB.Form Form1 
   ...
   Begin VB.ListBox List1 
      Height          =   1230
      ItemData        =   "LegoJoy.frx":0000
      Left            =   720
      List            =   "LegoJoy.frx":0002
      Sorted          =   -1  'True
      TabIndex        =   27
      Top             =   5040
      Width           =   2535
   ...
   Begin VB.Label Label12 
      Caption         =   "Lijst Topscores (hoogste beneden!)"
      Height          =   375
      Left            =   720
      TabIndex        =   28
      Top             =   4680
...
'To shorten time to communicate I made a version with only one send and 1 recieve per cycle
On Error GoTo except
  ...
  If deltaX > 50 Then
     motor = 2
    Else
       If deltaX < -50 Then motor = 1 Else motor = 3 'beide motoren
  End If
  If DeltaY < -50 Then
     direct = 0
   Else
  ...
  'If diroud <> direct Then
    commando = "dir " & direct & "," & motor 'richting
    vpb.Execute commando, LASM
  '  diroud = direct
  'End If
  If motoroud <> motor Then  'stoppen
    commando = "out 1," & motoroud  '1 is off/0=float
    vpb.Execute commando, LASM
    motoroud = motor
  End If
    commando = "out 2," & motor
    vpb.Execute commando, LASM  'aanzetten
  ...
  commando = "setv 31,2,1" 'variabele 31 met constavte(2) met waarde (1)
...
Private Sub ContrSens()
  ...
  schak = False
   ...
   'If nResult = 1 Then schak = True
   commando = "poll 9,2"  'waarde senor 2 opvragen schak"
   nResult = vpb.Execute(commando, LASM, nPos, result)
   If nResult = 1 Then schak = True
   commando = "poll 9,1"  'waarde senor 1 opvragen/licht
   nResult = vpb.Execute(commando, LASM, nPos, result)
...
Dim StrPunt As String
   ...
   'score berekenen
   punt = 1000 - Val(LblTijd.Caption) - Val(LblPunt.Caption)
   'op topscorelijst
   StrPunt = punt
   If StrPunt > Left(List1.List(List1.ListCount - 1), 3) Then MsgBox "gefeliciteerd je bent de nieuwe topscorer"
   List1.AddItem punt & "  " & Text3.Text & " " & Text4.Text & " " & Text5.Text
